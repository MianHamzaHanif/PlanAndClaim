...
export async function approveAndDepositFixPlan30(amountHuman) {
    await ensureChain("bscTestnet");
    ...
    // 1) Allowance check
    const allowance = await usdt.methods.allowance(from, FIX_PLAN30_ADDRESS).call();

    // 2) Approve if needed
    if (BigInt(allowance) < BigInt(amount)) {
        await usdt.methods.approve(FIX_PLAN30_ADDRESS, amount).send({ from });
    }

    // 3) Deposit to plan
    const receipt = await plan.methods.depositFixPlanP30(amount).send({ from });
    return receipt;
}
...
export async function getUserStakeLength(user) {
    ...
    const n = await c.methods.getUserStakeLengthP30(user).call();
    return Number(n || 0);
}
...
export async function getUserStakeDetails(user, index) {
    await ensureChain("bscTestnet");
    const c = getPlanContract();
    const d = await c.methods.userStakeDetailsFlixPlan30(user, index).call();
...
export async function simulateProcessAutoUnlockedStake(user, index, nowTs) {
    ...
    try {
        const res = await c.methods
            .processAutoUnlockedStakesIndexP30(user, index, String(nowTs))
            .call({ from: user });
...
export async function getSelfRewardFromUpLinerFixPlan(user, { chainKey = "bscTestnet" } = {}) {
    ...
    // let v;
    // try {
    //     v = await c.methods.getSelfRewardFromUpLinerFixPlan30(user).call();
    // } catch {
    //     // fallback if contract uses msg.sender (no arg)
    //     try { v = await c.methods.getSelfRewardFromUpLinerFixPlan30().call({ from: user }); }
    //     catch { v = "0"; }
    // }
    ...
    const v = await c.methods.getSelfRewardFromUpLinerFixPlan30(user).call();
    const y = await c.methods.gettotalWithdrawP30(user).call();
...
export async function getSelfRewardFromDownLinerFixPlan(user, { chainKey = "bscTestnet" } = {}) {
    ...
    // let v;
    // try {
    //     v = await c.methods.getSelfRewardFromDownLinerFixPlan30(user).call();
    // } catch {
    //     // fallback if contract uses msg.sender (no arg)
    //     try { v = await c.methods.getSelfRewardFromDownLinerFixPlan30().call({ from: user }); }
    //     catch { v = "0"; }
    // }
    // const raw = (v && (v.reward ?? v.amount ?? v.value ?? v[0])) ?? v ?? "0";
    // return String(raw || "0");
    ...
    const v = await c.methods.getSelfRewardFromDownLinerFixPlan30(user).call();
    const y = await c.methods.gettotalWithdrawP30(user).call();
...
export async function claimFixPlan30(typeNum, { chainKey = "bscTestnet" } = {}) {
    await ensureChain(chainKey);
    ...
    return await c.methods.claimFixPlan30(t).send({ from });
}
export const claim = {
    self: (opts) => claimFixPlan30(CLAIM_TYPES.SELF, opts),
    downline: (opts) => claimFixPlan30(CLAIM_TYPES.DOWNLINER, opts),
    upline: (opts) => claimFixPlan30(CLAIM_TYPES.UPLINER, opts),
};

export async function unstakeFixPlanP30({ from, index, chainKey = "bscTestnet" } = {}) {
    await ensureChain(chainKey);
    ...
    return await c.methods.unstakeFixPlanP30(i).send({ from });
}
