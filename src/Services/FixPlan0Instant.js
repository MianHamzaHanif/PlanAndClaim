...
export const FIX_PLAN0_ADDRESS = "0xDcd41740DF47061Ef827Ca0B515D32e02d57bfa1";
...
export async function approveAndDepositFixPlan0(amountHuman) {
    ...
    // 1) Allowance check
    const allowance = await usdt.methods.allowance(from, FIX_PLAN0_ADDRESS).call();

    // 2) Approve if needed
    if (BigInt(allowance) < BigInt(amount)) {
        await usdt.methods.approve(FIX_PLAN0_ADDRESS, amount).send({ from });
    }

    // 3) Deposit to plan
    const receipt = await plan.methods.depositFixPlanP0(amount).send({ from });
    return receipt;
}
...
export async function getUserStakeLength(user) {
    ...
    const n = await c.methods.getUserStakeLengthP0(user).call();
    return Number(n || 0);
}
...
export async function getUserStakeDetails(user, index) {
    await ensureChain("bscTestnet");
    const c = getPlanContract();
    const d = await c.methods.userStakeDetailsFlixPlan0(user, index).call();
...
export async function simulateProcessAutoUnlockedStake(user, index, nowTs) {
    ...
    try {
        const res = await c.methods
            .processAutoUnlockedStakesIndexP0(user, index, String(nowTs))
            .call({ from: user });
...
export async function getSelfRewardFromUpLinerFixPlan(user, { chainKey = "bscTestnet" } = {}) {
    ...
    // return String(raw || "0");
     ...
     if (!isValidAddress(user)) throw new Error("Invalid address");
        ...
        const v = await c.methods.getSelfRewardFromUpLinerFixPlan0(user).call();
        const y = await c.methods.gettotalWithdrawP0(user).call();
...
export async function getSelfRewardFromDownLinerFixPlan(user, { chainKey = "bscTestnet" } = {}) {
    ...
    // let v;
    // try {
    //     v = await c.methods.getSelfRewardFromDownLinerFixPlan0(user).call();
    // } catch {
    //     // fallback if contract uses msg.sender (no arg)
    //     try { v = await c.methods.getSelfRewardFromDownLinerFixPlan0().call({ from: user }); }
    //     catch { v = "0"; }
    // }
    // const raw = (v && (v.reward ?? v.amount ?? v.value ?? v[0])) ?? v ?? "0";
    // return String(raw || "0");
    ...
    if (!isValidAddress(user)) throw new Error("Invalid address");
        ...
        const v = await c.methods.getSelfRewardFromDownLinerFixPlan0(user).call();
        const y = await c.methods.gettotalWithdrawP0(user).call();
...
export async function claimFixPlan0(typeNum, { chainKey = "bscTestnet" } = {}) {
    await ensureChain(chainKey);
    ...
    return await c.methods.claimFixPlan0(t).send({ from });
}
export const claim = {
    self: (opts) => claimFixPlan0(CLAIM_TYPES.SELF, opts),
    downline: (opts) => claimFixPlan0(CLAIM_TYPES.DOWNLINER, opts),
    upline: (opts) => claimFixPlan0(CLAIM_TYPES.UPLINER, opts),
};
