...
export const FIX_PLAN90_ADDRESS = "0xf6a74b001b3667d7841b920f4C2a12A0D30e5954";
...
export async function approveAndDepositFixPlan90(amountHuman) {
    await ensureChain("bscTestnet");
    ...
    // 1) Allowance check
    const allowance = await usdt.methods.allowance(from, FIX_PLAN90_ADDRESS).call();

    // 2) Approve if needed
    if (BigInt(allowance) < BigInt(amount)) {
        await usdt.methods.approve(FIX_PLAN90_ADDRESS, amount).send({ from });
    }

    // 3) Deposit to plan
    const receipt = await plan.methods.depositFixPlanP90(amount).send({ from });
    return receipt;
}
...
export async function getUserStakeLength(user) {
    ...
    const n = await c.methods.getUserStakeLengthP90(user).call();
    return Number(n || 0);
}
...
export async function getUserStakeDetails(user, index) {
    await ensureChain("bscTestnet");
    const c = getPlanContract();
    const d = await c.methods.userStakeDetailsFlixPlan90(user, index).call();
...
export async function simulateProcessAutoUnlockedStake(user, index, nowTs) {
    ...
    try {
        const res = await c.methods
            .processAutoUnlockedStakesIndexP90(user, index, String(nowTs))
            .call({ from: user });
...
export async function getSelfRewardFromUpLinerFixPlan(user, { chainKey = "bscTestnet" } = {}) {
    ...
    // let v;
    // try {
    //     v = await c.methods.getSelfRewardFromUpLinerFixPlan90(user).call();
    // } catch {
    //     // fallback if contract uses msg.sender (no arg)
    //     try { v = await c.methods.getSelfRewardFromUpLinerFixPlan90().call({ from: user }); }
    //     catch { v = "0"; }
    // }
    ...
    const v = await c.methods.getSelfRewardFromUpLinerFixPlan90(user).call();
    const y = await c.methods.gettotalWithdraw90(user).call();
...
export async function getSelfRewardFromDownLinerFixPlan(user, { chainKey = "bscTestnet" } = {}) {
    ...
    // let v;
    // try {
    //     v = await c.methods.getSelfRewardFromDownLinerFixPlan90(user).call();
    // } catch {
    //     // fallback if contract uses msg.sender (no arg)
    //     try { v = await c.methods.getSelfRewardFromDownLinerFixPlan90().call({ from: user }); }
    //     catch { v = "0"; }
    // }
    // const raw = (v && (v.reward ?? v.amount ?? v.value ?? v[0])) ?? v ?? "0";
    // return String(raw || "0");
        ...
        const v = await c.methods.getSelfRewardFromDownLinerFixPlan90(user).call();
        console.log("vvvvv yyy",v)
        const y = await c.methods.gettotalWithdraw90(user).call();
        console.log("ya log yyy yy", y)
...
export async function claimFixPlan90(typeNum, { chainKey = "bscTestnet" } = {}) {
    await ensureChain(chainKey);
    ...
    return await c.methods.claimFixPlan90(t).send({ from });
}
export const claim = {
    self: (opts) => claimFixPlan90(CLAIM_TYPES.SELF, opts),
    downline: (opts) => claimFixPlan90(CLAIM_TYPES.DOWNLINER, opts),
    upline: (opts) => claimFixPlan90(CLAIM_TYPES.UPLINER, opts),
};

export async function unstakeFixPlanP90({ from, index, chainKey = "bscTestnet" } = {}) {
    await ensureChain(chainKey);
    ...
    return await c.methods.unstakeFixPlanP90(i).send({ from });
}
