...
const Flexible = () => {
  ...
  const handleClaim = async (typeNum) => {
    ...
    try {
      setClaiming(true);
      setTxMsg("Claiming…");
      const receipt = await claimFixPlan0(typeNum, { chainKey: "bscTestnet" });
  ...
  const handleUnstake = async () => {
    try {
      ...
      setUnstaking(true);
      setTxMsg("Unstaking…");
      const receipt = await unstakeFixPlanP0({ from: account, index: idx, chainKey: "bscTestnet" });
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  if (viewMode === "self") {
                    ...
                    // just signal TeamLevelTable to reload
                    setRefreshTick((t) => t + 1);
                    // (optional) still refresh the dashboard stats
                    await refreshDashboardStats(account);
                  }
                } finally {
                  setRefreshing(false);
                  ...
                  <div className="tab-pane fade" id="profile-tab-pane" role="tabpanel" aria-labelledby="profile-tab" tabIndex={0}>
                    <div className="card w-100">
                      ...
                      <div className="mx-auto">
                        <button
                          type="button"
                          className="zv-cta zv-cta--sm mt-3"
