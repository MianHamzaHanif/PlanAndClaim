...
import {
  ...
  unstakeFixPlanP480,
  isValidAddress,
} from "../../Services/FixPlan480Instant";
...
const Flexible = () => {
  ...
  const handleApproveAndStake = async () => {
    ...
    try {
      ...
      const receipt = await approveAndDepositFixPlan480(amount);
  ...
  const handleClaim = async (typeNum) => {
    ...
    try {
      setClaiming(true);
      setTxMsg("Claiming…");
      const receipt = await claimFixPlan480(typeNum, { chainKey: "bscTestnet" });
  ...
  const handleUnstake = async () => {
    try {
      ...
      setUnstaking(true);
      setTxMsg("Unstaking…");
      const receipt = await unstakeFixPlanP480({ from: account, index: idx, chainKey: "bscTestnet" });
