...
import {
  ...
  unstakeFixPlanP90,
  isValidAddress,
} from "../../Services/FixPlan90Instant";
...
const Flexible = () => {
  ...
  const [teamRefreshTick, setTeamRefreshTick] = useState(0); // ping to refresh the team table
  ...
  const handleApproveAndStake = async () => {
    ...
    try {
      ...
      const receipt = await approveAndDepositFixPlan90(amount);
  ...
  const handleClaim = async (typeNum) => {
    ...
    try {
      setClaiming(true);
      setTxMsg("Claiming…");
      const receipt = await claimFixPlan90(typeNum, { chainKey: "bscTestnet" });
  ...
  const handleUnstake = async () => {
    try {
      ...
      setUnstaking(true);
      setTxMsg("Unstaking…");
      const receipt = await unstakeFixPlanP90({ from: account, index: idx, chainKey: "bscTestnet" });
