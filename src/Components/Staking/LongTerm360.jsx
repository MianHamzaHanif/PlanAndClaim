...
import {
  ...
  unstakeFixPlanP360,
  isValidAddress,
} from "../../Services/FixPlan360Instant";
...
const Flexible = () => {
  ...
  const handleApproveAndStake = async () => {
    ...
    try {
      ...
      const receipt = await approveAndDepositFixPlan360(amount);
  ...
  const handleClaim = async (typeNum) => {
    ...
    try {
      setClaiming(true);
      setTxMsg("Claiming…");
      const receipt = await claimFixPlan360(typeNum, { chainKey: "bscTestnet" });
  ...
  try {
    ...
    setUnstaking(true);
    setTxMsg("Unstaking…");
    const receipt = await unstakeFixPlanP360({ from: account, index: idx, chainKey: "bscTestnet" });
