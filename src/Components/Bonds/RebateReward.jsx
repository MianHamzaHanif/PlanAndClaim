...
import { getUserClaimDetails, getUserClaimsLength, getUserDirectDetails, getUserDirectLength, getUserPendingClaimedAmount, getUserRebateDetails, getUserRebateLength, getUserWithdrawDetails, getUserWithdrawLength, withdrawByDays } from "../../Services/turbine";
...
const RebateReward = () => {
    ...
    const withdrawHistory = async (addr = account) => {
        ...
        try {
            ...
            const count = await getUserRebateLength(addr);
            ...
            const rows = await Promise.all(
                idxs.map(async (i) => {
                    const d = await getUserRebateDetails(addr, i);
                    const amountFmt = formatUnits(String(d.amount ?? "0"), tokenDecimals, 4);
                    return {
                        index: i,
                        amountRaw: String(d.amount ?? "0"),
                ...
                <div className="container px-md-3">
                    <div className="row g-4">
                        ...
                        <div className="col-md-12">
                            <div className="Heading Heading2in mb-4">Rebate Reward History</div>
